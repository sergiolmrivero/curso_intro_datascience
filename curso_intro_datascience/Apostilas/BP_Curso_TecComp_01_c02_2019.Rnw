...
\section{O ciclo de tratamento e anÃ¡lise de dados}
...
\subsection{Vetores}
O vetor Ã© um objeto matemÃ¡tico caracterizado em um conjunto de segmentos orientdos de reta que possuem o mesmo mÃ³dulo, direÃ§Ã£o e sentido. Ele contÃªm elementos de classes diferentes, conforme apresentado abaixo:


Vetor de classe numÃ©rica:
<<echo=TRUE>>=
a <- c(1,3500,5.3,543,-2,4000)
@
Vetor de classe de caractÃ©r:
<<echo=TRUE>>=
b <- c("taxa de juros","taxa de cÃ¢mbio","reservas bancÃ¡rias") 
@
Vetor de classe lÃ³gica:
<<echo=TRUE>>=
c <- c(FALSE,TRUE,FALSE,TRUE) 
@
\subsection{Matrizes}
SÃ£o compostas por linhas (valores ordenados na horizontal) representadas pela letra "m" e colunas (valores ordenados na vertical) representadas pela letra "n", onde os dados sÃ£o convertidos segundo essas disposiÃ§Ãµes.
<<echo=TRUE>>=
...
Onde:
       \begin{description}
       \item [data:] parÃ¢metro que representa os dados usados para criar matriz;
       \item [nrow:] parÃ¢metro para nÃºmero de linhas;
       \item [ncol:] parÃ¢metro para nÃºmero de colunas.
       \end{description}
